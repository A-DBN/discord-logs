...
export default new Event({
    ...
    run: async client => {
        ...
        client.user?.setActivity("AreiTTV on Twitch", { type: ActivityType.Watching });
        const embed = new EmbedBuilder()
            .setTitle("Redemarrage du bot")
            .setColor(Number(0x5ac18e))
            .setDescription(
                `Redemarrage du bot terminé, si le bot redemarre regulierement ou après une certaine action => mp ZenkiuD#3333`
            )
            .setTimestamp();
